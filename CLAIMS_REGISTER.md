...
| AUX-010 | Health endpoint reflects real backend readiness (not static healthy JSON only) | README.md | Platform | AuricruxApiController | GET /api/health | PARTIAL |
...
| AUX-014 | All 25/25 tests prove production AI capabilities (not mock endpoint wiring) | TEST_REPORT_FINAL_25-25.md | QA | tests | Re-run against real model | FAIL |
...
| AUX-021 | Authentication (OAuth2/OIDC) on web and mobile | README Security | Platform | Web / Mobile | Auth gate | FAIL |
...
| AUX-028 | HTTPS + security headers + CORS + rate limiting are real prod controls | TEST_REPORT; DEPLOYMENT | Platform | Web host | Header/rate probes | PARTIAL |
| AUX-029 | Client ApiEndpoint configurable and wired to a non-mock production backend | README.md | Platform | appsettings / MauiProgram | Chat against prod | PARTIAL |
